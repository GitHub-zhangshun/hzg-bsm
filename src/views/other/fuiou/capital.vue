    ...
    <div class="post-main-container">
      ...
      <el-table stripe @row-dblclick="pitchRow" ref="table" :data="table.list" border style="width: 100%;margin-top:5px;" v-loading="table.loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="名称" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{ scope.row.enterpriseName || scope.row.realName }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="userType" label="用户类型">
          <template slot-scope="scope">
            <span>{{ getDicTitleById(scope.row.userType) }}</span>
          </template>
        </el-table-column>
          ...
          <div class="order">
            <section>
              ...
              <div class="content">
                ...
                <div class="item"><span>账户管理费: {{numberFormat(info.serviceCharge)}}</span></div>
        ...
        switch (action.routeUrl) {
          case 'execute_export':
            ...
            if (sure) {
              ...
              const header_key = ['订单号', '姓名', '手机', '类型', '用户类型', '金额', '余额', '操作时间', '状态', '备注']
              const header_value = ['txnSsn', 'name', 'phone', 'type', 'userType', 'amount', 'balance', 'operationAt', 'status', 'remarks']
              if (page > 0) {
