  ...
  <div class="post-container">
    <div class="post-main-container">
      ...
      <el-table stripe @row-dblclick="pitchRow" ref="table" :data="table.list" border style="width: 100%;margin-top:5px;" v-loading="table.loading" @selection-change="handleSelectionChange">
          ...
          <template slot-scope="props">
            <el-form label-position="left" inline class="table-expand">
              <el-form-item label="已还本金">
                <span>{{ getObjVal(props.row, 'loanStatistics_alsoPrincipal', 'nf')}}</span>
              </el-form-item>
              <el-form-item label="待还本金">
                <span>{{ getObjVal(props.row, 'loanStatistics_waitPrincipal', 'nf')}}</span>
              </el-form-item>
              <el-form-item label="已还利息">
                <span>{{ getObjVal(props.row, 'loanStatistics_alsoInterest', 'nf')}}</span>
              </el-form-item>
              <el-form-item label="待还利息">
                <span>{{ getObjVal(props.row, 'loanStatistics_waitInterest', 'nf')}}</span>
              </el-form-item>
              <el-form-item label="已还违约金 ">
                <span>{{ getObjVal(props.row, 'loanStatistics_alsoDamages', 'nf')}}</span>
              </el-form-item>
              <el-form-item label="待还违约金">
                <span>{{ getObjVal(props.row, 'loanStatistics_waitDamages', 'nf')}}</span>
              </el-form-item>
              <el-form-item label="已收管理费">
                <span>{{ getObjVal(props.row, 'loanStatistics_alsoServiceCharge', 'nf')}}</span>
              </el-form-item>
              <!-- <el-form-item label="待还管理费 ">
                <span>{{ getObjVal(props.row, 'loanStatistics_waitServiceCharge', 'nf')}}</span>
              </el-form-item> -->
            </el-form>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="idcardCode" label="身份证号" show-overflow-tooltip>
        </el-table-column>
        ...
        <el-table-column label="存管开户" show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column prop="bankPhone" label="银行预留手机" show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column label="开户时间" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{getTime(scope.row.openTime)}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="待还金额">
          <template slot-scope="scope">
            <span>{{numberFormat(scope.row.loanStatistics && numAdd(scope.row.loanStatistics.waitPrincipal || 0, scope.row.loanStatistics.waitInterest || 0, scope.row.loanStatistics.waitDamages || 0))}}</span>
          </template>
        </el-table-column>
        <el-table-column label="已还金额">
          <template slot-scope="scope">
            <span>{{numberFormat(scope.row.loanStatistics && numAdd(scope.row.loanStatistics.alsoPrincipal || 0, scope.row.loanStatistics.alsoInterest || 0, scope.row.loanStatistics.alsoDamages || 0))}}</span>
          </template>
        </el-table-column>
      ...
      <el-dialog :close-on-click-modal="false" v-el-drag-dialog :title="dialogTitle" :visible.sync="dialogBaseStatus" @close="dialogBaseClose">
        <el-form :model="dialogBaseForm" ref="dialogBaseForm" :rules="baseFormRules.dialogRules" label-width="100px">
          <el-form-item label="是否关系户" prop="isRelation">
      ...
      <el-dialog :close-on-click-modal="false" v-el-drag-dialog :title="dialogTitle" :visible.sync="dialogAddStatus" @close="dialogAddClose">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="dialogSubmit('add')" :loading="dialogBaseButtonDisabled">确 定</el-button>
        </div>
      </el-dialog>
    </div>
  </div>
      ...
      func_dialog(action, row) {
        ...
        switch (action.routeUrl) {
          ...
          case 'dialogAdd':
            ...
            this.dialogAddForm.isEnterprise = false
            break;
          default:
            break;
        }
        ...
        switch (action.routeUrl) {
          ...
          case 'execute_export':
            ...
            if (sure) {
              ...
              const header_key = ['姓名', '是否关系户', '手机号', '身份证', '电子合同', '存管开户', '开户时间', '银行卡', '助贷机构', '借款金额', '待还金额', '已还金额', '账户余额']
              const header_value = ['realName', 'isRelation', 'phone', 'idcardCode', 'isContract', 'isOpen', 'openTime', 'bankBranchName', 'providerName', 'totalLoan', 'waitMoney', 'alsoMoney', 'usable']
              if (page > 0) {
