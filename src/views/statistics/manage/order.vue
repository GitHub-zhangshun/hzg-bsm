  ...
  <div class="post-container">
    <div class="post-main-container">
      ...
      <el-table stripe @row-dblclick="pitchRow" ref="table" :data="table.list" border style="width: 100%;margin-top:5px;" v-loading="table.loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="realName" label="用户姓名" show-overflow-tooltip>
        ...
        <el-table-column prop="rate" label="产品利率">
          <template slot-scope="scope">
        ...
        <el-table-column prop="amount" label="出借金额">
          <template slot-scope="scope">
        ...
        <el-table-column label="优惠券">
          <template slot-scope="scope">
            <span>{{ scope.row.couponType && ( scope.row.couponType == 1 ? '返利券' : '加息券' ) || '无' }} {{scope.row.couponVal}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="createAt" label="投标时间" show-overflow-tooltip>
          <template slot-scope="scope">
    ...
    methods: Object.assign(_func(), {
      async func_execute(action, row) {
        switch (action.routeUrl) {
          case 'execute_export':
            let days = this.diffTime(this.searchForm.operationStarTime, this.searchForm.operationEndTime)
            if (days > 90) {
            ...
            if (sure) {
              ...
              const header_key = ['订单号', '用户姓名', '手机号码', '标的名称', '产品利率', '标的状态', '投标金额', '预期收益', '优惠券', '投标时间', '担保方']
              const header_value = ['txnSsn', 'realName', 'phone', 'bidName', 'rate', 'bidStatus', 'amount', 'expectIncome', 'coupon', 'createAt', 'providerName']
              if (page > 0) {
                for (let index = 0; index < page; index++) {
                  ...
                  await this.getExportData({
                    ...
                    try {
                      // 数据格式化
                      data.result.map(i => {
                        i.expectIncome = i.expectIncome && this.numAdd(i.expectIncome || 0)
                        i.createAt = i.createAt && this.getTime(i.createAt)
                        i.bidStatus = i.bidStatus && this.getDicTitleById(i.bidStatus)
                        i.coupon = i.couponType && `${( i.couponType == 1 ? '返利券' : '加息券' ) || '无'}${i.couponVal}`
                      })
                    } catch (error) {
                      console.error(error)
                    }
                    this.exportExcel({
                      title: `${index+1}-和掌柜-出借订单记录`,
                      header_key,
                      header_value,
                      data: data.result,
                      num: index,
