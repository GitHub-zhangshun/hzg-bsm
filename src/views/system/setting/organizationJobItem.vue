<template>
  <div class="post-container">
    <div class="post-main-container">
      <el-tag type="danger" style="width:100%">所属机构:{{$route.query.name}}</el-tag>
      ...
      <el-table stripe @row-dblclick="pitchRow" ref="table" :data="table.list" border style="width: 100%;margin-top:5px;" v-loading="table.loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="isSuper" label="是否管理者">
          <template slot-scope="scope">
            <span>{{scope.row.isSuper?'管理者':'非管理'}}</span>
          </template>
        </el-table-column>
      ...
      <el-dialog :close-on-click-modal="false" v-el-drag-dialog :title="dialogTitle" :visible.sync="dialogBaseStatus" @close="dialogBaseClose">
        <el-form :model="dialogBaseForm" ref="dialogBaseForm" label-width="100px">
          ...
          <el-form-item label="是否管理者" prop="isSuper">
            <el-select v-model="dialogBaseForm.isSuper" placeholder="请选择">
    ...
    methods: Object.assign(_func(), {
          ...
          case 'dialogEdit':
            if (row.length !== 1) {
              ...
              this.$request({
                ...
                data.result.isSuper = data.result.isSuper !== null && data.result.isSuper.toString()
      ...
      dialogSubmit(name) {
        switch (name) {
          case 'base':
            // 添加机构字段
            this.dialogBaseForm.organizationId = this.$route.query.id
            this.$request({
