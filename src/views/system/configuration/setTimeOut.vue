  ...
  <div class="post-container">
    <div class="post-main-container">
      ...
      <el-table stripe @row-dblclick="pitchRow" ref="table" :data="table.list" border style="width: 100%;margin-top:5px;" v-loading="table.loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="cron" label="表达式" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="implementUrl" label="执行地址" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="requestUrl" label="请求地址" show-overflow-tooltip>
        </el-table-column>
        <el-table-column label="是否启用">
          <template slot-scope="scope">
            <span>{{scope.row.isStart?'启用':'禁用'}}</span>
          </template>
        </el-table-column>
      ...
      <el-dialog :close-on-click-modal="false" v-el-drag-dialog :title="dialogTitle" :visible.sync="dialogBaseStatus" @close="dialogBaseClose">
        <el-form :model="dialogBaseForm" ref="dialogBaseForm" label-width="100px">
          ...
          <el-form-item label="表达式" prop="cron">
            <el-input v-model="dialogBaseForm.cron"></el-input>
          </el-form-item>
          <el-form-item label="执行地址" prop="implementUrl">
            <el-input v-model="dialogBaseForm.implementUrl"></el-input>
          </el-form-item>
          <el-form-item label="请求地址" prop="requestUrl">
            <el-input v-model="dialogBaseForm.requestUrl"></el-input>
          </el-form-item>
          <el-form-item label="排序" prop="sort">
            <el-input v-model="dialogBaseForm.sort"></el-input>
          </el-form-item>
          <el-form-item label="是否启用" prop="isStart">
            <el-select v-model="dialogBaseForm.isStart" placeholder="请选择">
          ...
          case 'dialogEdit':
            if (row.length !== 1) {
              ...
              this.$request({
                ...
                this.dialogBaseStatus = true
                this.dialogBaseForm.isStart = data.result.isStart.toString()
              })
            }
            break
