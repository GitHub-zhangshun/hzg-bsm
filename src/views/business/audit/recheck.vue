  ...
  export default {
    ...
    data() {
      return {
        ...
        tableListQuery: {
          realStatus: 'bid_product_real_audit_recheck' // 初审状态
        }
      }
    },
    created() {
        ...
        switch (name) {
          case 'base':
            this.$refs['dialogBaseForm'].validate((valid) => {
              if (valid) {
                let statusSimplify = this.dialogBaseForm.status === 'true' ? 'bid_product_real_audit_publish' : 'bid_product_real_audit_recheck_fail'
                this.$request({
